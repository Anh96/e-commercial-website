...
function render_product_inshop(products){
    ...
    const header_link_sale_off_container = $$('.header-link-sale-off-container');
    for(let i =0;i<products.length;++i){
        if(products[i].percent_saleoff==0){
           header_link_sale_off_container.forEach(headItem=>{
    ...
    for(let i=0;i<b4etd.length;++i){
        ...
        var total_width = b4etd.length*x;
        ...
        $('.right-arr').onclick = ()=>{
            ...
            if(x*6*click_mount<total_width){
                $('.top-search-overflow').scrollLeft += x*6 +47;
            }
            if(x*6*click_mount==total_width - x*6*(click_mount-1)){
